...
mon_canton <- surfaces_ra1852 %>% 
  ...
  pivot_longer(-c(code, canton, departement)) %>% ### synthaxe - le vecteur des champs qu'on ne veut pas permutter
  select(-c(code, canton, departement)) %>% 
  print()
```

Alors quelle est le type de terre labourable la plus étendue ? A Lille, ce sont les céréales.  (mais avec quand-même 12 000 ha de prairies à l'époque !) 
Avez-vous un peu de vigne ? NON (snif)
...
```{r}
#| label: verif_mon_canton
#| warning: false

verif_totaux <- mon_canton %>%
  pivot_wider() %>% 
  mutate(TOTAL_TL = sum(cereales,racines_et_legumes_divers, cultures_diverses, prairies_artificielles, jacheres) ,
         ECART_TL=TOTAL_TL-total_des_terres_labourables) %>% 
    mutate(ECART_TOT = superficie_totale -(total_des_terres_labourables +  prairies_naturelles + vignes + autres_cultures_arborescentes + paturages + autres_surfaces_ecoumene  ))
...
```
curl 'https://geo.api.gouv.fr/communes?nom=Wasquehal&fields=departement&boost=population&limit=5'
```
...
On est content d'avoir des informations sur Lille. Mais nous constatons que nous avons également des informations sur d'autres villes qui ont Lille dans leur nom comme Saint-André-les-Lille...   
...
On obtient bien les coordonnées du centre de Lille : "coordinates":[3.0469,50.6311].
Attention : les coordonnées données envoyées par le terminal via API sont inversées !!!

Pour vérifier on peut utiliser google maps, en mettant directement les coordonnées dans l'url après le @ : 
[https://www.google.fr/maps/@3.0469,50.6311.5z](https://www.google.fr/maps/@50.6311,3.0469.13z) (le 13.5z est le zoom)
...
table_departement1852<-surfaces_ra1852 %>% 
  filter(is.na(departement)==TRUE) %>% 
  mutate(codeNew=code/10, #pour enlever le zero de la fin (car format numérique)
         codeNew1=as.character(codeNew), #transforme en caractère pour appliquer les fonctions str
         codeNew2=str_pad(codeNew1, 2,pad="0",side="left")) %>% 
  select(DEP=codeNew2, DEP_LIB=canton)
  
#jointure pour récupérer les codes départements

table_canton1852<-surfaces_ra1852 %>% 
  filter(is.na(departement)==FALSE) %>% 
  left_join(table_departement1852, by = c("departement"="DEP_LIB")) %>% 
  mutate(CANTON = str_replace_all(canton, " ", "-")) #on remplace les blancs par des tirets

liste_canton<-table_canton1852 %>% pull(CANTON)

#stats RA1852 région HDF

RA1952_HDF <-table_canton1852 %>% 
  filter(DEP %in%c("02","59","62","80","60")) %>% 
  summarise(SAU_TOT=sum(superficie_totale))
...
surfaces_ra1852_coordonnees_centre <- table_canton1852 %>%
  rowwise() %>%
...
###identification des NA

canton_NULL <-surfaces_ra1852_coordonnees_centre %>% 
  filter(is.na(centre)==T)
...
#| warning: false
#| echo: true
#nettoyage des coordonnées géo X et Y, puis séparation en deux champs

tableRA1852_latlong <-surfaces_ra1852_coordonnees_centre_corrigees %>%
  mutate(centre = as.character(centre)) %>% 
  mutate(COORDO = str_replace_all(centre, "c", "")) %>%#on remplace les c( et ) par rien
  mutate(COORDO=str_replace_all(COORDO, "[)]", "")) %>% #ATTENTION : les parenthèses doivent être entre crochets sinon erreur
  mutate(COORDO=str_replace_all(COORDO, "[(]", "")) %>% 
  mutate(longitude=str_split_fixed(COORDO, ",",2)[,1]) %>%  #la fonction str_plit_fixed permet de séparer une colonne en "2" à partir du caractère ",". le [,1] permet de lui préciser quelle est la colonne qu'il doit retenir pour la latitude
  mutate(latitude=str_split_fixed(COORDO, ",",2)[,2]) %>%  #idem ici avec la 2eme colonne
...
autre doc Leaflet en français : https://lrouviere.github.io/TUTO_R/leaflet.html
...
leaflet(data = tableRA1852_latlong) %>% #transforme la table en objet geographique
  addTiles() %>% #ajoute le fond de carte openstreetmap (par défaut)
  addMarkers(lng = ~longitude, #lng transforme la donnée en longitude pour geolocaliser
             lat = ~latitude, #lng transforme la donnée en latitude pour geolocaliser
             popup = ~canton, #champ que tu veux representer dans la fenetre pop-up
             label = ~canton) #étiquette que tu veux mettre dans la carte
...
surfaces_ra1852_geo <- st_as_sf(tableRA1852_latlong %>% 
                                drop_na(c(longitude, latitude)),
                                coords = c("longitude", "latitude"),
                                crs = 4326) #4326 ça doit certainement correspondre à la projection utilisée dans l'API /openStreetMap
##WGS84 (EPSG 4326) : OpenStreetMap uses the WGS84 spatial reference system used by the Global Positioning System (GPS). It uses geographic coordinates between -180° and 180° longitude and -90° and 90° latitude. So this is the "native" OSM format.
## Les coordonnées GeoJson issues de l'API sont bien en WGS84 (EPSG4326)
...
### ouverture de la couche des departements (téléchargées en format JSON de data.gouv)
dep <- sf::read_sf(here('data/dep2021_simplify2.json'), crs=2154) 
### enlever outre-mer
dep_metro <- dep %>% 
  rowwise() %>% 
  filter(nchar(dep)==2) %>% #car DOM sont à 3 chiffres
  ungroup()

### ne garder que les coutours de la France et les fusionner
dep_union <- st_union(dep_metro)
plot(dep_union)

### ne garder les voronoi uniquement qui sont en intersection avec les contours de la France
...
data_voronoi <- data.frame(id = 1:length(voronoi_surfaces_ra1852))# pour attribuer un n°identifiant en 2 temps avec un cbind qui colle les colonnes
